...
最近一次性能验收记录：CPU `0.77%`、峰值内存 `362.8 MB`、3D 空闲帧率 `30.07 FPS`。该数据用于版本回归，不代表所有硬件上的固定占用。

特殊三屏排列、旋转屏、不同显卡驱动，以及用户自有 Live2D 模型仍应在目标电脑上人工验证。

## 开源与贡献

快捷宠自身源码采用 [MIT License](./LICENSE)。欢迎通过 [Issues](../../issues) 提交可复现的问题、功能建议或兼容性结果。提交问题时请附上 Windows 版本、显示器排列和缩放、桌宠模式、模型类型与复现步骤。
